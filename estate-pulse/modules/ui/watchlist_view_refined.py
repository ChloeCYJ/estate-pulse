...
def render_watchlist_page(
    *,
    ...
    st.title("투자 후보")
    st.caption("관심 단지와 관심 매물을 등록하고, 단지·평형 기준 후보를 간단히 비교합니다.")
    ...
    with add_complex_col:
        ...
        if complexes:
            ...
            selected_complex = st.selectbox("단지", list(complex_options.keys()), key="watchlist_complex_v2")
    ...
    with add_listing_col:
        ...
        if listings:
            ...
            selected_listing = st.selectbox("매물", list(listing_options.keys()), key="watchlist_listing_v2")
            ...
            display_rows.append(
                {
                    ...
                    "판단": _candidate_decision_label(row),
                    "최근 분석일": _format_datetime_or_dash(row.get("latest_analysis_date")),
                }
            )
    ...
    if not relevant_events:
        st.caption("현재 확인할 주요 정책 이벤트가 없습니다.")
        return

    event_df = pd.DataFrame(relevant_events)[
        ["effective_from", "effective_to", "policy_type", "title", "impact_level", "status"]
    ].rename(
        columns={
            "effective_from": "시작일",
            "effective_to": "종료일",
...
def _candidate_decision_label(row: dict) -> str:
    if not row.get("analysis_available"):
        return "분석 불가"
    shortage_cash = float(row.get("shortage_cash") or 0)
    return "매수 가능" if shortage_cash <= 0 else "조건부 가능"
